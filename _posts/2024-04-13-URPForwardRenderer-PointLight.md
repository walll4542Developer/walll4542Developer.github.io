...
이 연구는 유니티 2021 버전 기준으로 작성되었음을 미리 알립니다. 
{: .notice--info}
...
(가용 인적 자원 부족이나, 유니티 버전 업데이트에 대응을 못한다던가, 안정성 또는 유지보수 문제로 수정 하면 안되는 경우같은 어른의 이유)

유니티 로직에 따라서 라이트 갯수가 최대치 이상이면 라이트 인덱스(Light Index) 순서에 따라 선입선출(First In, First Out)으로 라이트를 렌더링 하지 않습니다.
