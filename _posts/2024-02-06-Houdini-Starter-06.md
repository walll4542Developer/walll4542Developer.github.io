...
트랜스폼 노드의 스케일 파라미터 값에 음수를 입력하여 포인트들의 좌표가 ${(x,y) (-x,y) (x,-y) (-x,-y)}$ 가 되도록 하고 바운드 노드와 연결하면 위 이미지 처럼 중앙을 ${(0, 0, 0)}$ 으로 대칭하는 테이블탑을 만들 수 있습니다.
...
다음은 다리의 두께를 결정해줄 LegThick 파라미터를 추가합니다. 이를 Leg 의 모든 바운드 노드의 Lower Padding / Upper Padding 값에 파라미터로 연결해줍니다.
