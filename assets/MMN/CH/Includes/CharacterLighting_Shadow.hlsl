...
//-----------------------------------------------------------------------------
half GetReceivedShadow(half3 lightDirWS, float3 positionWS, float3 characterCenterPos,
    half visualHeight, half topShadow, half bottomShadow)
{
    half cloudShadow = MMN_GlobalTex_CloudShadows(positionWS.xyz).r;
    ...
    half gradientWidth = visualHeight * 0.5;

    half3 lightTangent = cross(lightDirWS, half3(0, 1, 0));
    lightTangent = cross(lightDirWS, lightTangent);

    half shadowGradient = dot(lightTangent, positionWS - characterCenterPos);
    shadowGradient = saturate(shadowGradient / gradientWidth + 0.5);

    half receivedShadow = lerp(topShadow, bottomShadow, shadowGradient);
    receivedShadow = min(receivedShadow, cloudShadow);
    receivedShadow = min(1.0, smoothstep(0.0, 1.0, saturate(receivedShadow * 2.0 - 0.5)) + 0.2 /*최소 밝기 보정*/);
...
half GetReceivedShadow2(half3 lightDirWS, float3 positionWS, float3 characterCenterPos, half visualHeight,
    half topShadow, half bottomShadow)
{
    half cloudShadow = MMN_GlobalTex_CloudShadows(positionWS.xyz).r;
    ...
    half spreadAmount = visualHeight * 4.0;
    half3 centerOffset = half3(0, -0.5, 0);
    half lightMask = dot(half3(1, -1, 1) * spreadAmount * (characterCenterPos - positionWS) + centerOffset, lightDirWS);
    half receivedShadow = lerp(bottomShadow, topShadow, saturate(lightMask));
    receivedShadow = min(receivedShadow, cloudShadow);
