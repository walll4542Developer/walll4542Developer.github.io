    ...
    float3 vertexLightColor = float3(0.0, 0.0, 0.0);
        ...
        Light light = GetAdditionalLight(lightIndex, positionWS);
        ...
        #endif
            {
                ...
                float volumeShading = 0.25;
            #else // defined(_SHADINGTYPE_STANDARD)
                float volumeShading = saturate(nDotL + 0.9) * 0.32;
            #endif

                float3 finalColor = volumeShading * lightingMask * attenuatedLightColor;
                vertexLightColor += max(0.0, finalColor);
            }
        LIGHT_LOOP_END
    #endif
    ...
    float3 cameraDirWSFlatten, float rimBand)
{
    ...
    resultColor += max(0.0, finalColor);
