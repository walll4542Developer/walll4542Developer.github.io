                         ...
                         out MainLightShadingResult result)
{
    ...
    float turnOffFlatShading = (1.0 - input.flatShadingOff);
...
#if (defined(_SHADINGTYPE_SKINBODY) || defined(_SHADINGTYPE_SKINFACE) || defined(_SHADINGTYPE_STOCKINGS))
    {
        ...
        finalShading = lerp(finalShading, finalShading + flatteness, input.verticalGradientRemapped * turnOffFlatShading);
    ...
    // giColor의 강도를 30% 과장한다. 데이터 전체를 고칠 수 없기 때문에 여기서 수정하는 것이다.
    result.ambientLightResult = saturate(ambientLightResult * input.lightingData.giColor * 1.3);
}
