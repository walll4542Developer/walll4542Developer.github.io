...
float4 BasePassFragment(Varyings input) : SV_Target
{
    ...
    resultColor.rgb = ProcessCharacterColor(inputData,
        ...
        _SHADINGTYPE_SKINBODY_VALUE, _FlatShadingOff, flatShadingAmount, dyedBaseColor, _SilhouetteOff, _SilhouetteTintColor);

    #ifdef _OUTLINE_FEATURE
        float3 outlineColor = OnePassOutline(inputData, mainLight.direction, characterData, verticalGradientRemapped, _OutlineColorMode, _SHADINGTYPE_SKINBODY_VALUE);
