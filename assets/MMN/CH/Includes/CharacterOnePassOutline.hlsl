...
float3 OnePassOutlineColor(float outlineColorMode, float shadingType, float verticalGradientRemapped)
{
    ...
    return saturate(outlineColor);
}

float3 OnePassOutline(InputData inputData, float3 lightDir, CharacterData characterData, float verticalGradientRemapped, float outlineColorMode, float shadingType)
{
    float outlineWidth = OnePassOutlineWidth(inputData, lightDir, shadingType);
    float3 outlineColor = OnePassOutlineColor(outlineColorMode, shadingType, verticalGradientRemapped);
