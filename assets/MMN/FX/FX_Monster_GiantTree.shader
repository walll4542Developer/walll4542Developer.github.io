Shader "MMN/FX/FX_Monster_GiantTree"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float2 JitterUV (float2 uv, float randomSeed)
			{
			...
			float4 frag(Varyings input) : SV_Target
			{
				...
				float dataTexR0 = SAMPLE_TEXTURE2D(_DataTex, sampler_DataTex, float2(time, frac(randomSeed * 0.5 + 0.5))).r;
				float dataTexR1 = SAMPLE_TEXTURE2D(_DataTex, sampler_DataTex, float2(time, frac(randomSeed * 0.5))).r;
				float3 dataTexGBA = SAMPLE_TEXTURE2D(_DataTex, sampler_DataTex, float2(time, randomSeed)).gba;
				float4 dataTex = float4(dataTexR0, dataTexR1, dataTexGBA.gb);
				...
				float3 eyeTex = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, eyeUV).rgb;
				float dataTexG = SAMPLE_TEXTURE2D(_DataTex, sampler_DataTex, eyeUV).g; // 동공 마스킹
				...
				float3 eyeTexG = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, eyePupilUV).rgb;
				dataTexG = SAMPLE_TEXTURE2D(_DataTex, sampler_DataTex, eyePupilUV).g;

				eyeTex = lerp(eyeTex, eyeTexG, dataTexG);
				float eyeTexAlpha = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, uv.zw).a;
				...
				float3 color = eyeTex * lightColor;
				float alpha = 1;
