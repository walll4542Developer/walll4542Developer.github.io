...
{
    float transition = saturate(abs(1 - spawnTransition)); // 정규화된 트랜지션이 아닌 거리 값(1 초과)이 들어와서 음수가 되도 양수로 바꿔줍니다.
    transition = lerp(transition, 1, unity_OrthoParams.w); // Ortho 에서는 동작 안하게 합니다.

    if (blendMode >= 1.0 && blendMode <= 2.0)
    {
        // AdditiveColor, AdditiveAlpha
        finalColor.rgb *= transition;
    }
    // else if (blendMode <= 6)
    // {
    //     // Multiply
...
float3 SceneColor(float2 screenPos)
{
    float3 sceneColor = SampleSceneColor(screenPos);
    return sceneColor;
}
