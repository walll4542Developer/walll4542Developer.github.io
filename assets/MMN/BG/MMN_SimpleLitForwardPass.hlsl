...
float4 LitPassFragmentSimple(Varyings input, FRONT_FACE_TYPE isFacing : FRONT_FACE_SEMANTIC) : SV_Target
{
    ...
    //float Subtractive 라이팅
    #if float_SUBTRACTIVE_LIGHTMAP_ON
        color = UniversalFragmentLightCustomBaked(inputData, diffuse, specular, smoothness, emission, alpha, /* normalTS */ float3(0, 0, 1), /*shadowDimming*/ 0, /*floatLambertWeight*/ _halfLambertWeight, _BackfaceReceiveShadowOff, isFacing, _BackFaceNormalturn);
    #else
        color = UniversalFragmentLightCustom(inputData, diffuse, specular, smoothness, emission, alpha, /* normalTS */ float3(0, 0, 1), /*shadowDimming*/ 0, /*floatLambertWeight*/ _halfLambertWeight, _BackfaceReceiveShadowOff, isFacing, _BackFaceNormalturn);
    #endif
    ...
    // float heightDarkGradiation = distance(input.positionOS.rgb , float3(0, _CenterPointHeight, 0));
    // innerAO = saturate(pow(max(0, innerAO / _AOarea), _AOintensity));
    // float heightDarkGradiation = saturate(abs(posObjectNormal.g));


    float heightDarkGradiation = UniversalFragmentTreeLeavesInnerAO(input.positionOS.gggg, _CenterPointHeight, _GroundAOarea, _GroundAOintensity, 1);
    color *= heightDarkGradiation;
