    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                //폼
                float foamDepth = max(0.3 - 3.0 * waterDepth, waterDepth + _FoamOffset);
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                //폼
                float foamDepth = max(0.2- 3.0 * waterDepth, waterDepth + _FoamOffset);
                float foamDepthDistortion = (1.0 - (0.3 + distortion.r / 5.0 + distortion.b / 10.0)) / (distortion.g * 0.1 + 0.1);
                float drawFoam = (1 - saturate(pow(saturate(saturate(foamDepth) *  foamDepthDistortion ), 15.0)));
                float foamCoeff = drawFoam * _FoamOpacity;
                // return drawFoam;
                ...
                color.rgb = lerp(scatterColor.rgb, _FoamColor.rgb, saturate(foamCoeff));
                float opacity = saturate(foamCoeff + depthCoeff);
