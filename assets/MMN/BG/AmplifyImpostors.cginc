...
CBUFFER_END
    ...
    #define AI_INV_TWO_PI  UNITY_INV_TWO_PI		//0.15915494309f
    #define AI_PI          UNITY_PI				//3.14159265359f
    #define AI_INV_PI      UNITY_INV_PI			//0.31830988618f
#endif
...
inline void OctaImpostorFragment(inout SurfaceOutputSimpleLit o, out float4 clipPos, out float3 worldPos, float4 uvsFrame1, float4 uvsFrame2, float4 uvsFrame3, float4 octaFrame, float4 interpViewPos)
{
    float depthBias = -1.0;
...
inline void SphereImpostorFragment(inout SurfaceOutputSimpleLit o, out float4 clipPos, out float3 worldPos, float4 frameUV, float4 viewPos)
{
    #if _USE_PARALLAX_ON
    ...
    #if !defined(AI_RENDERPIPELINE) // no S
        #if defined(SHADOWS_DEPTH)
            if (unity_LightShadowBias.y == 1.0) // get only the shadowcaster, this is a hack
...
//뜯어봅시다 : SphereImpostorVertex 를 개조해서 , 가로로만 도는 빌보드 임포스터를 만들어 보았습니다.

inline void HorizontalImpostorVertex(inout float4 vertex, inout float3 normal, inout float4 frameUVs, inout float4 viewPos)
{
    // INPUTS
    float2 uvOffset = _AI_SizeOffset.zw;
    float sizeX = _FramesX;
    ...
    float2 sizeFraction = fractions.xy;
    float axisSizeFraction = fractions.z;
    float2 fractionsUVscale = 1 / _ImpostorSize;
    ...
    float3 objectCameraPosition = mul(ai_WorldToObject, float4(worldCameraPos, 1)).xyz - _Offset.xyz; //ray origin
    float3 objectCameraDirection = normalize(objectCameraPosition) * float3(1, 0, 1); // 카메라가 위로 올라간걸 인식시키지 않는다

    // Create orthogonal vectors to define the billboard
    float3 upVector = float3(0, 1, 0);
    float3 objectHorizontalVector = normalize(cross(objectCameraDirection, upVector));
    float3 objectVerticalVector = upVector; //위아래 계산 벡터연산 없애고 그냥 업벡터로 처리

    // Create vertical radial angle
    // 가로 세로 타일링 UV 제어 . 세로 타일링 제어는 새로 만들어준것임. 원래 것을 삭제하려 했는데 서로 꼬여 연결되어 있는게 많아서.
    // 카메라가 월드 X 를 바라볼 때부터 시작합니다.
    // 위에서 Y를 -1로 상수 조정한 것을 여기서 다시 복구했습니다. 원본을 삭제하고 싶었지만 다른데서 쓰고 있는걸 수정하기 싫어서
    //atan2( -objectCameraDirection.z, -objectCameraDirection.x ) * AI_INV_TWO_PI 이게 한 바퀴를 나타냅니다.
    float verticalAngle = frac(atan2(-objectCameraDirection.z, -objectCameraDirection.x) * AI_INV_TWO_PI) * (sizeX * (sizeY + 1)) ;
    float HorizontalAngle = - (atan2(-objectCameraDirection.z, -objectCameraDirection.x) * AI_INV_TWO_PI) * (sizeY + 1);
    

    // Create horizontal radial angle : 가로로 도는 부분
    float verticalDot = dot(objectCameraDirection, upVector);
    float yRot = sizeFraction.x * AI_PI * verticalDot * (2 * frac(verticalAngle) - 1);

    // Billboard rotation : Y축으로만 돌게 만듭니다.
    float3 uvExpansion = vertex.xyz;
    float cosY = cos(yRot);
    float sinY = sin(yRot);
    // float2 uvRotator = mul( uvExpansion, float2x2( cosY, -sinY, sinY, cosY ) );
...
inline void HorizontalImpostorFragment(inout SurfaceOutputSimpleLit o, out float4 clipPos, out float3 worldPos, float4 frameUV, float4 viewPos)
{
    #if _USE_PARALLAX_ON
        float4 parallaxSample = tex2Dbias(_Normals, float4(frameUV.xy, 0, -1));
        frameUV.xy = ((0.5 - parallaxSample.a) * frameUV.zw) + frameUV.xy;
