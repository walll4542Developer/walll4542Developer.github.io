...
Varyings DepthNormalsVertex(Attributes input)
{
    ...
    // float3 viewDirWS = distance ( GetCameraPositionWS() ,vertexInput.positionWS );
    ...
    // #else
    //     output.normalWS = float3(NormalizeNormalPerVertex(normalInput.normalWS));
    // #endif
...
float4 DepthNormalsFragment(Varyings input) : SV_TARGET
{
    ...
    //     float3 packedNormalWS = PackFloat2To888(remappedOctNormalWS);      // values between [ 0,  1]
    //     return float4(packedNormalWS, 0.0);
    // #else
    float2 uv = input.uv;

    // #if defined(_NORMALMAP)
        float3 normalTS = SampleNormal(uv, TEXTURE2D_ARGS(_BumpMap, sampler_BumpMap));
        float3 normalWS = TransformTangentToWorld(normalTS, float3x3(input.tangentWS.xyz, input.bitangentWS.xyz, input.normalWS.xyz));
    // #else
