...
float4 LitPassFragmentSimple(Varyings input) : SV_Target
{
    ...
    //밤 낮 변환 Emission
    float3 emissionColor = night2DayControl(_EmissionColorBright.rgb, _EmissionColorDark.rgb, _OutsideInside, _TempNight2DaySwitchTest);
    ...
    float3 emission = diffuse * emissionColor * alpha * diffuseAlpha.a;
    ...
    if (unity_LODFade.x != 0)
    {
        ...
        Unity_Dither_linear(fadeValue, input.screenPos, lodFade, 0.9);
        clip(lodFade);
    }
    else
    {
